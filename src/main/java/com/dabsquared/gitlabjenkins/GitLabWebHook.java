...
public class GitLabWebHook implements UnprotectedRootAction {
    ...
    public void getDynamic(final String projectName, final StaplerRequest req, StaplerResponse res) {
        ...
        final Iterator<String> restOfPathParts =
                Splitter.on('/').omitEmptyStrings().split(req.getRestOfPath()).iterator();
        ...
        if (!paths.isEmpty() && paths.get(0).equals("refs")) {
            ...
            if (index == -1) { index = paths.lastIndexOf("builds"); }
            if (index == -1) { index = paths.lastIndexOf("!builds"); }
    ...
    private void generateStatusPNG(String branch, String commitSHA1, Job project, final StaplerRequest req,
            final StaplerResponse rsp) throws ServletException, IOException {
        SCMTriggerItem item = SCMTriggerItems.asSCMTriggerItem(project);
        GitSCM gitSCM = getGitSCM(item);
        ...
        try {
            URL resourceUrl =
                    new URL(Jenkins.getInstance().getPlugin("gitlab-plugin").getWrapper().baseResourceURL + imageUrl);
    ...
    protected void buildOpenMergeRequests(GitLabPushTrigger trigger, Integer projectId, String projectRef) {
        try {
            ...
            String tailUrl =
                    GitlabProject.URL + "/" + projectId + GitlabMergeRequest.URL + "?state=opened&per_page=100";
            ...
            for (org.gitlab.api.models.GitlabMergeRequest mr : mergeRequests) {
                if (projectRef.endsWith(mr.getSourceBranch()) || (
                        trigger.getTriggerOpenMergeRequestOnPush().equals("both") && projectRef
                                .endsWith(mr.getTargetBranch()))) {
                    ...
                    if (mr.getAuthor() != null) { newReq.getObjectAttribute().setAuthorId(mr.getAuthor().getId()); }
            ...
            if (mergeRecord == null) {
                ...
                if (b != null && b.getMarked() != null && b.getMarked().getSha1String().equals(commitSHA1)) {
                    if (triggeredByMergeRequest == isMergeBuild) {
                        LOGGER.log(Level.FINE,
                                build.getNumber() + " Build found matching " + commitSHA1 + " " + (isMergeBuild
                                        ? "merge" : "normal") + " build");
                        return build;
                    }
                }
